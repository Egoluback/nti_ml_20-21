...
for target in targets:
    ...
    for channelIndex in range(len(channels)):
        ...
        params_channels.append(grid["learning_rate"][errors.index(min(errors))])
        print(params_channels)

    with open("params/params_learning_rate_{0}".format(target), "w+") as file:
